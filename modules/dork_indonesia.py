...
def generate_indonesia_dorks(email_in, phone_data, username_in, name_in):
    ...
    dengan logika sintaks yang rapi dan valid.
    """
    ...
    # 3. Mention Berita & Forum Publik (Dengan Tanda Kurung Grup)
    ...
    # 4. Global Footprint (Pencarian Tanpa Batasan Situs Media)
    if target_user:
        q_global = f'"{target_user}"'
        dorks.append({
            "title": "🌐 Pindai Jejak Digital Global (Username di Seluruh Web)",
            "query": q_global,
            "link": f"https://www.google.com/search?q={quote_plus(q_global)}"
        })
